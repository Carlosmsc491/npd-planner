...
def process_one(path: Path, birefnet, refiner, ref_size: int, device: str,
    ...
    name = path.name
    st.step(name, idx, "Cargando imagen…", 5)
    orig = Image.open(path).convert("RGB")
    ow, oh = orig.size
    orig_rgb = np.asarray(orig)

    st.step(name, idx, "BiRefNet — recorte base (2048)…", 25)
    ...
    st.step(name, idx, "Refinador — quitando base y basura…", 60)
    ...
    st.step(name, idx, "Afinando bordes (guided filter)…", 75)
    ...
    st.step(name, idx, "Limpiando fleco blanco y basura flotante…", 88)
    ...
    st.step(name, idx, "Recorte cuadrado 3600 y guardado…", 95)
...
def main() -> int:
    ...
    st.step("—", 0, "Cargando modelos (BiRefNet + refinador)…", 0)
    ...
    st.d["current"] = {}
    st.write()
    print("Recortes listos.", flush=True)
