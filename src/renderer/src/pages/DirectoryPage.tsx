...
export default function DirectoryPage() {
  ...
  const [filterContactFor, setFilterContactFor] = useState('')
  ...
  const allContactFor = useMemo(() => {
    const set = new Set<string>(CONTACT_FOR_SUGGESTIONS)
    contacts.forEach((c) => c.contactFor.forEach((v) => set.add(v)))
  ...
  const filtered = useMemo(() => {
    ...
    return contacts.filter((c) => {
      ...
      const matchFilter =
        !filterContactFor || c.contactFor.includes(filterContactFor)
      return matchSearch && matchFilter
    })
  }, [contacts, search, filterContactFor])
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-6 py-3 border-b border-gray-200 dark:border-gray-700 shrink-0">
            ...
            <select
              ...
              onChange={(e) => setFilterContactFor(e.target.value)}
              className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-green-500/30"
            >
              <option value="">All Clients/Locations</option>
              {allContactFor.map((v) => (
                <option key={v} value={v}>{v}</option>
              ))}
            </select>
            {(search || filterContactFor) && (
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 dark:text-gray-500">
                ...
                <p className="text-sm font-medium">
                  {search || filterContactFor ? 'No contacts match your filters' : 'No contacts yet'}
                </p>
                {!search && !filterContactFor && (
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-gray-50 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Contact For</th>
                    {columns.map((col) => (
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                  {filtered.map((contact) => (
                    <tr key={contact.id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {contact.contactFor.length > 0
                            ? contact.contactFor.map((v) => (
                                <span key={v} className="inline-flex items-center rounded-full bg-green-100 dark:bg-green-900/30 px-2 py-0.5 text-xs text-green-700 dark:text-green-400">
                                  {v}
                                </span>
                              ))
...
function ContactDrawer({ contact, columns, uid, onClose, onSaved, onError }: ContactDrawerProps) {
  ...
  async function handleSave() {
    ...
    try {
      if (contact) {
        await updateDirectoryContact(contact.id, {
  ...
  return (
    <div className="w-96 shrink-0 border-l border-gray-200 dark:border-gray-700 flex flex-col bg-white dark:bg-gray-800">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            {form.contactFor.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {form.contactFor.map((v) => (
                  <span key={v} className="inline-flex items-center gap-1 rounded-full bg-green-100 dark:bg-green-900/30 px-2 py-0.5 text-xs text-green-700 dark:text-green-400">
                    {v}
                    <button onClick={() => removeContactFor(v)} className="hover:text-red-500">
                      <X size={10} />
                    </button>
                  </span>
                ))}
        ...
        {columns.map((col) => (
          <Field key={col.id} label={col.name}>
            {col.type === 'text' && (
              <input
                value={(form.customValues[col.id] as string) ?? ''}
                onChange={(e) => setCustomValue(col.id, e.target.value)}
                className={INPUT_CLS}
              />
            )}
            {col.type === 'droplist' && (
      ...
      <div className="px-5 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
        <button onClick={onClose} className="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          Cancel
        </button>
        <button
          onClick={handleSave}
...
function ColumnManager({
  ...
  function addOption(colId: string) {
    setColumns((prev) =>
      prev.map((c) =>
        c.id === colId
          ? { ...c, options: [...c.options, { id: nanoid(6), label: '' }] }
          : c
      )
    )
  }
  ...
  function updateOption(colId: string, optId: string, label: string) {
    setColumns((prev) =>
      prev.map((c) =>
        c.id === colId
          ? { ...c, options: c.options.map((o) => (o.id === optId ? { ...o, label } : o)) }
          : c
      )
    )
  }
  ...
  async function handleSave() {
    const valid = columns.every((c) => c.name.trim())
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg max-h-[80vh] flex flex-col rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <button onClick={onClose} className="rounded p-1 text-gray-400 hover:text-gray-600"><X size={16} /></button>
        </div>
          ...
          {columns.map((col) => (
            <div key={col.id} className="rounded-xl border border-gray-200 dark:border-gray-700 p-3 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <select
                  value={col.type}
                  onChange={(e) => updateColumn(col.id, { type: e.target.value as DirectoryColumnType, options: [] })}
                  className={INPUT_CLS}
                >
                ...
                <div className="space-y-1 pl-2">
                  {col.options.map((opt) => (
                    <div key={opt.id} className="flex items-center gap-2">
                      <input
                        value={opt.label}
                        onChange={(e) => updateOption(col.id, opt.id, e.target.value)}
                        placeholder="Option label"
                        className={`${INPUT_CLS} flex-1 text-xs`}
                      />
                      <button onClick={() => removeOption(col.id, opt.id)} className="text-gray-400 hover:text-red-500">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
          <button onClick={onClose} className="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
            Cancel
          </button>
          <button
            onClick={handleSave}
