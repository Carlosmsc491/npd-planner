...
// Every function has try/catch ΓÇö never let Firestore errors crash silently
...
export const SLEEVE_BY_PRICE_DEFAULTS: Record<string, string> = {
  '$7.99':   '0.25',
  '$11.99':  '0.30',
  '$12.99':  '0.30',
  '$14.99':  '0.35',
...
export async function updateRecipeFileId(
  ...
  try {
    ...
    if (snap.empty) return   // file was never tracked ΓÇö nothing to update
...
export async function updateRecipeHeartbeat(
  ...
  try {
    ...
    // Heartbeat failures are non-fatal ΓÇö log only
...
export async function addRecipeActivity(
  ...
  try {
    ...
    // No lanzar error ΓÇö la actividad es secundaria, no debe romper el flujo
...
/** Delete a note (author or admin only ΓÇö enforced in UI, not here). Decrements count if active. */
...
export async function repairActiveNotesCount(
  ...
  try {
    ...
    // Silently ignore ΓÇö this is a best-effort background repair
  }
}
